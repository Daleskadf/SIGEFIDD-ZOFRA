    ...
    public partial class EditarDocumento : Page
    {
                ...
                using (var cn = new SqlConnection(connStr))
                {
                    ...
                    using (var cmd = new SqlCommand(sql, cn))
                    {
                        ...
                        using (var dr = cmd.ExecuteReader())
                        {
                            if (!dr.Read())
                            {
                                MostrarMsg("No se encontró el documento.", false);
                                return;
                            }

                            // Parsear código documento (formato: CODIGO-NUMERO-AÑO)
        ...
        protected void btnEnviarCorreccion_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                {
                    MostrarMsg("ID de documento inválido.", false);
                    return;
                }
                ...
                using (var cn = new SqlConnection(connStr))
                {
                    ...
                    using (var transaction = cn.BeginTransaction())
                    {
                        try
                        {
                            ...
                            // Registrar en historial que el registrador levantó corrección
                            ...
                            using (var cmd = new SqlCommand(sqlHistorial, cn, transaction))
                            {
                                ...
                                cmd.Parameters.AddWithValue("@detalle", "Registrador levantó corrección sobre observaciones.");
                                cmd.ExecuteNonQuery();
                            }
        ...
        private string BuildNav(string rol)
        {
            ...
            return
                ...
                "<a href='CargarDocumento.aspx' class='nav-item'>" + svgCargar + "Cargar Documento</a>" +
                "<a href='MisDocumentos.aspx' class='nav-item active'>" + svgMisDocs + "Mis Documentos</a>" +
                "<a href='../GestionRoles/GestionRoles.aspx' class='nav-item'>" + svgRoles + "Gestión de Roles</a>" +
